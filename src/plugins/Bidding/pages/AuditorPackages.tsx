...
export default function AuditorPackages(): JSX.Element {
				...
				action: (
					<>
						<IconButton
							...
							icon="info-circle"
							color="success"
							onClick={() => { history.push(`/bidding/auditor/package/${item.code}`) }}
						/>
					</>
				)
			}
	...
	const refresh = async () => {
		...
		try {
			setPackages(await packageService.getAuditorPackages(status));
		}
		catch {
