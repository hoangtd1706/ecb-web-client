...
export default function Experts(): JSX.Element {
	...
	const EXPERT_ROLE: RoleType = 'EXPERT_ROLE';
	...
	const handleCreate = async () => {
		...
		try {
			await userService.create(EXPERT_ROLE, userNumber);
	...
	const handleRemove = async (_userNumber: string) => {
		dispatch(loadingActions.show());
		try {
			await userService.remove(EXPERT_ROLE, _userNumber);
			dispatch(alertActions.show(alertMessage.ACTION_SUCCESS, 'success'));
			refresh();
		}
		catch {
	...
	return (
		<Frame title="Danh sách chuyên quản">
			<Grid container alignItems="flex-end" spacing={1}>
				...
				<Grid item xs={12}>
					{users.filter(x => x.role === EXPERT_ROLE).map((user, index) => (
						<Chip
							style={{ margin: '4px' }}
							key={index}
