...
export default function Auditors(): JSX.Element {
	...
	const handleCreate = async () => {
		...
		try {
			await userService.create(AUDITOR_ROLE, userNumber);
	...
	const handleRemove = async (_userNumber: string) => {
		dispatch(loadingActions.show());
		try {
			await userService.remove(AUDITOR_ROLE, _userNumber);
			dispatch(alertActions.show(alertMessage.ACTION_SUCCESS, 'success'));
			refresh();
		}
		catch {
	...
	return (
		<Frame title="Danh sách kiểm tra">
			<Grid container alignItems="flex-end" spacing={1}>
				...
				<Grid item xs={12}>
					{users.filter(x => x.role === AUDITOR_ROLE).map((user, index) => (
						<Chip
							style={{ margin: '4px' }}
							key={index}
