...
export default function Viewers(): JSX.Element {
	...
	const VIEWER_ROLE: RoleType = 'VIEWER_ROLE';
	...
	const handleCreate = async () => {
		...
		try {
			await userService.create(VIEWER_ROLE, userNumber);
	...
	const handleRemove = async (_userNumber: string) => {
		dispatch(loadingActions.show());
		try {
			await userService.remove(VIEWER_ROLE, _userNumber);
			dispatch(alertActions.show(alertMessage.ACTION_SUCCESS, 'success'));
			refresh();
		}
		catch {
	...
	return (
		<Frame title="Danh sách người dùng">
			<Grid container alignItems="flex-end" spacing={1}>
				...
				<Grid item xs={12}>
					{users.filter(x => x.role === VIEWER_ROLE).map((user, index) => (
						<Chip
							style={{ margin: '4px' }}
							key={index}
